...
const fetchAllBlogCategoryBlogs = async (allBlogObject, page) => {
  try {
    const response = await fetch(
      `https://dev.to/api/articles?page=${page}&per_page=1000`
    );
...
const fetchTopCategoryBlogs = async (topObject) => {
  try {
    const response = await fetch(
      `https://dev.to/api/articles/latest?per_page=10&top=10`
    );
    const data = await response.json();
...
const fetchTrendingCategoryBlogs = async (trendingObject) => {
  try {
    const response = await fetch(
      `https://dev.to/api/articles?per_page=10&top=5`
    );
    const data = await response.json();
...
export const fetchAllCategoryBlogs = async (setBlogs) => {
  try {
    ...
    const allTagsIncludedFinalAllCategoryBlogs =
      await getAllCategoryBlogsWithAllTags(finalAllCategoryBlogs);
